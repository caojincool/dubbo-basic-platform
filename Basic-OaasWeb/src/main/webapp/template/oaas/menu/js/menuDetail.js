...
ruizhi.oaas.menuDetail = function() {
	
	//私有成员
	var form1;
	var _this = this;
	var URL =  WEB_ROOT + '/oaas/funcPage/menu/menuDetail.do';//当前窗口的URL
	//私有方法
	var checkCode;
	return{
		/* 初始化 */
		init : function() {
			...
			var menu = paramObj.menu;
			
			var catalog = paramObj.catalog;
			
			form1 = new ruizhi.FormExt("oaas-menuDetail-form1");
			
			//新增
			if(ruizhi.IsNull(menu)){
				form1.objectToForm(catalog);
			}
			//修改
			else{
				form1.getObject('menuCode').prop("disabled",true);
				form1.objectToForm(menu);
			}
		},	
		...
		//单选目录
		selCatalog : function(){
			var url = WEB_ROOT + '/oaas/funcPage/menu/selCatalog.do';
			var width = null;
			var heigth = null;
			var paramObj = {};//传给模态窗口的参数
			var submitFn = ruizhi.oaas.menuDetail.selCatalogComeBack;//模态窗口提交后回调方法
