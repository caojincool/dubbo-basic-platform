...
ruizhi.DataGrid = function(dataGridId, initParam,disableEdit) {
...
//			    	 }
			...
			loadComplete : function(e) {
                var _this = this,
                    gridNum = e;
                if (gridParams.customPager && customPager == null){
                    var html = '<div class="col-md-4">\
						       		<label for=""><select class="rowNum" style="border: 1px solid #999999;border-radius: 4px;height: 28px;width: 55px;">\
										<option value="全部">全部</option>\
										<option value="10">10</option>\
										<option value="20">20</option>\
										<option value="50">50</option>\
									</select>每页，</label>\
						       		<label for="">共 <span class="records"></span> 条记录</label>\
							   </div>\
							   <div class="pagination col-md-4">\
							        <ul class="pagination-ul pagination-sm"></ul>\
							    </div>\
							    <div class="total col-md-4" style="text-align: right">\
							       <label for="">共 <span class="pages"></span> 页</label>\
							       <label for="">去第 <input class="toPage" type="num" style="border: 1px solid #999999;border-radius: 4px;height: 24px;width: 30px;"> 页 <a class="go">GO</a></label>\
							   </div>'
                    $(gridParams.customPager).addClass('customPager').append(html);
                    var pageParams = {
                        totalPages: gridNum.total,
                        visiblePages: 5,
                        next: '>',
                        prev: '<',
                        first: '首页',
                        ...
                        onPageClick: function (event, page) {
                            ...
                            var $pageInput = $(_this).parents('.ui-jqgrid.ui-widget.ui-widget-content.ui-corner-all').find('.ui-pg-input');
                            $pageInput.val(page);
                            $pageInput.trigger(e);//模拟页码框按下回车
                        }
                    }
                    $(gridParams.customPager).find('.rowNum').val(gridParams.rowNum);
                    $(gridParams.customPager).find('.pages').text(gridNum.total);
                    $(gridParams.customPager).find('.records').text(gridNum.records);
                    $(gridParams.customPager).appendTo($(_this).parents('.ui-jqgrid.ui-widget.ui-widget-content.ui-corner-all'));

                    customPager = $(gridParams.customPager).find('.pagination-ul').twbsPagination(pageParams);
                    $(gridParams.customPager).find('.go').click(function (){
                        var page = $(gridParams.customPager).find('.toPage').val();
                        page = page > gridNum.total ? gridNum.total : page
                        var e = jQuery.Event("keypress");//模拟一个键盘事件
                        e.keyCode = 13;//keyCode=13是回车
                        var $pageInput = $(_this).parents('.ui-jqgrid.ui-widget.ui-widget-content.ui-corner-all').find('.ui-pg-input');
                        $pageInput.val(page);
                        $pageInput.trigger(e);//模拟页码框按下回车
                        customPager.twbsPagination('show', parseInt(page));
                    });
                    $(gridParams.customPager).find('.rowNum').change(function (){
                        var page = $(this).val();
                        var $pageSelect = $(_this).parents('.ui-jqgrid.ui-widget.ui-widget-content.ui-corner-all').find('.ui-pg-selbox');
                        $pageSelect.val(page).change();
                        customPager = null;
                        $(gridParams.customPager).empty();
                        gridParams.rowNum = page
                    });
                }
			},
			resizeStop:function(newwidth, index){
				var width = $(".colum_"+dataGridId).width();
				var oldWidth = $("#sum_"+dataGridId).find("td:eq("+index+")").width();
				debugger;
				$(".colum_"+dataGridId).width(width+newwidth-oldWidth);
				$("#sum_"+dataGridId).width($("table[aria-labelledby='gbox_"+dataGridId+"']").width()-1);
			/*	width = $("#gview_"+dataGridId+ " .ui-common-table th:eq(0)").width()-1;
				console.log(width);
				$("#sum_"+dataGridId).find("td:eq(0)").width(width);*/
				$("#sum_"+dataGridId).find("td:eq("+index+")").css("width",newwidth);
			}
		};
...
//			if(!ruizhi.IsNull(height) && pHeight < height){
...
//			alert("宽度：" + width);
		...
		//编辑单元格事件失去焦点回调
		if(!ruizhi.IsNull(savesuccessfunc)){
