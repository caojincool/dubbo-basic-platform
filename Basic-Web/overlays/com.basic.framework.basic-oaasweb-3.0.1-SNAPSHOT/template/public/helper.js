...
//		return ruizhi.DateToString(this, true);
//	};
//	
		...
		DateToString : function(d, useTime) {
			if (d) {
				return d.getFullYear()
						+ "-"
						+ ruizhi.StrZeroAdd(d.getMonth() + 1)
						+ "-"
						+ ruizhi.StrZeroAdd(d.getDate())
						+ ((useTime) ? " " + ruizhi.StrZeroAdd(d.getHours()) + ":"
								+ ruizhi.StrZeroAdd(d.getMinutes()) + ":"
								+ ruizhi.StrZeroAdd(d.getSeconds()) : "");
			} else {
				return "";
			}
		},
...
//				height:'800px'
			...
			$(".modal-header").prepend("<div class='module'>"+$(".navigation-first-link-ul .actived span").text()+"</div>")
...
//			$('#'+obj.eleId).remove();
		...
		InvokeMethod:function(url,paramObj){
			...
			$.ajax({
				...
				dataType: 'json',
				success : function(msg) {
					if(!ruizhi.IsNull(msg)){
						if(msg.returnCode == '999'){//后台抛出异常
							if(!ruizhi.IsNull(msg.returnMsg)){
								ruizhi.Alert(msg.returnMsg);
							}else{
								ruizhi.Alert("操作失败");
							}
						...
						if(ruizhi.IsNull(msg.returnData)){//兼容以前代码外面并没有套处理结果DealResult那一层
							ret = msg;
						}else{
							ret = msg.returnData;
						}
		...
		openModalWin : function(name) {
			...
			var submitFn = ruizhi.doSomeThing;//模态窗口提交后回调方法
		...
		TableDictTrans:function(tableCode,colCode,colValue){
			if(_TableDictData[tableCode]==null){
				...
				var retList = ruizhi.InvokeMethod(url,obj);
				if(retList!=null){
		...
		TableDictGetValueList : function(tableCode, colCode){
			...
			if(ruizhi.IsNull(colCode)){
				var tableObj={};
				for(key in obj){
					tableObj[key]=ruizhi.ResolveToList(obj,key);
				}
				return tableObj;
			}

			
			return ruizhi.ResolveToList(obj,colCode);
		},
		...
		GetGidValue : function(gidCode){
			if(ruizhi.IsNull(gidCode)){
				ruizhi.Alert("gidCode不能为空");
				return ;
			}
			...
			var gidValue = null;
			var obj = ruizhi.InvokeMethod(url,paramObj);
			if(!ruizhi.IsNull(obj)){
		...
		GetNow : function(){
			...
			if(!ruizhi.IsNull(obj)){
				...
				ruizhi.Alert("获取服务器当前时间失败，转用当前系统时间！");
				date = new Date();
			}
			return date;
		},
		...
		Print : function(Vflag){
			var flag = false;
			if(ruizhi.IsNull(Vflag) || 'true' == Vflag){
				flag = true;
			}
			if(!flag){
...
//			if(!ruizhi.IsNull(privates) && privates.indexOf(privCode) > -1){
//				flag = true;
//				return flag;
//			}else{
//				flag = false;
...
//			}
		...
		GenButton : function (buttonGroup){
			...
			var isAdmin = ruizhi.IsSuperadminRole();
			if(!isAdmin){
				for(var i=0 ; i< buttons.length; i++){
					if(ruizhi.HasPriv(buttons[i]) ){
			...
			if(!ruizhi.IsNull(hasButton)){
				
				var url = WEB_ROOT + "/oaas/func/func/getFunc.do";
				var paramObj = {
						funcCodes : hasButton.join(",")
				};
				var data = ruizhi.InvokeMethod(url,paramObj);
				if(!ruizhi.IsNull(data)){
					//a标签id
					var a_ids = buttonGroup.a_ids;
					var html = '';
					for(var i in data ){
						var a_id = "";
						if(a_ids != null && i < a_ids.length){
							a_id = a_ids[i];
						}
						html += '<li><a id="' + a_id + '" href="javascript:'+data[i].funcUrl+'"><i class="'+data[i].funcIcon +' position-left"></i>'+data[i].funcName+'</a></li>'
					}
					ruizhi.GetObj(buttonGroup.id).html(html);
				}
			}
