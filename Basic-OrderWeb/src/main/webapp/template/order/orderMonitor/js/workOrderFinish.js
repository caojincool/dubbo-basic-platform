...
ruizhi.order.workOrderFinish = function() {
	
	var form1 = null;
	var _this = this;
	var URL =  WEB_ROOT + '/order/funcPage/orderMonitor/workOrderFinish.do';//当前窗口的URL
	...
	return{
		...
		init : function() {
			...
			var workOrderFinish = paramObj.workOrderFinish;
			//alert("模态窗口内获取初始化参数："+ruizhi.ToJson(orderType));
			
			form1 = new ruizhi.FormExt("order-workOrderFinish-form1");
			
			form1.objectToForm(workOrderFinish);
		},	
		...
		doSubmit : function() {
			...
			if(ruizhi.IsNull(_UserDetail.orgId) || ruizhi.IsNull(_UserDetail.orgName)){
				...
				valueObj.operOrgId = _UserDetail.orgId;//操作人默认组织
				valueObj.operOrgName = _UserDetail.orgName;//操作人默认组织名称
			}
...
ruizhi.ExecWait(function(){
	ruizhi.order.workOrderFinish.init()
});