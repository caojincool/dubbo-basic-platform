//////////////////////////////////////////
//任务派发规则实例
ruizhi.Package("ruizhi.order");

ruizhi.order.woDispRuleInst = function() {

	var grid1;
	var form1;
	var _UserDetail = ruizhi.GetUser();
	
	return {
		//初始化
		init : function() {

			grid1 = new ruizhi.DataGrid("order-woDispRuleInst-grid1",{
				postData : {
					
				},
				url : WEB_ROOT + '/order/func/woDispRuleInst/pagin.do'
			});
			form1 = new ruizhi.FormExt("order-woDispRuleInst-form1");
		},
		
		//加载数据
		loadData : function(ruleId) {
			var url = WEB_ROOT + '/order/func/woDispRuleInst/pagin.do';
//			var paramObj = form1.formToObject();
			if(!ruizhi.IsNull(ruleId)){
				var paramObj = {"ruleId":ruleId};
				grid1.loadData(url, paramObj);
			}else{
				var paramObj = form1.formToObject();
				grid1.loadData(url, paramObj);
			}
		},
		
		//新增窗口
		create : function() {
			var paramObj = {};//传给模态窗口的参数
			var url = WEB_ROOT + '/order/funcPage/woDispRuleInst/woDispRuleInstDetail.do';
			var width = null;
			var heigth = null;
			var woDispRuleInst = {};
			woDispRuleInst.pageDateType = 'CREATE';
			paramObj.woDispRuleInst = woDispRuleInst;
			var submitFn = ruizhi.order.woDispRuleInst.createComeBack;//模态窗口提交后回调方法

			var modalClass = "modal-lg";
			ruizhi.ShowModalWin(url,width,heigth,paramObj,submitFn,modalClass);
		},
		
		//新增窗口回调函数
		createComeBack : function(param){
			var params = {};
			param = ruizhi.ToJson(param);
			params.params = param;
			//alert("从模态窗口返回的参数："+ruizhi.ToJson(params));
			var url = WEB_ROOT + "/order/func/woDispRuleInst/createOrModify.do";
			ruizhi.InvokeMethodAsyn(url,params, ruizhi.order.woDispRuleInst.loadData);
			//ruizhi.order.woDispRuleInst.loadData();
		},
		
		//编辑窗口
		modify : function() {
			var paramObj = {};//传给模态窗口的参数
			var url = WEB_ROOT + '/order/funcPage/woDispRuleInst/woDispRuleInstDetail.do';
			var width = null;
			var heigth = null;
			var submitFn = ruizhi.order.woDispRuleInst.modifyComeBack;//模态窗口提交后回调方法
			var modalClass = "modal-lg";

			var ids = grid1.getCheckedIds();
			if(!ruizhi.IsNull(ids)){
				if(ids.length == 0){
					ruizhi.Alert('请选择');
					return false;
				}else if(ids.length > 1){
					ruizhi.Alert('不能选择多条数据');
					return false;
				}
			}
			var selItem = grid1.getSelectedItem();//所选行
			if(!ruizhi.IsNull(selItem)){
				selItem.pageDateType = 'UPDATE';
				paramObj.woDispRuleInst = selItem;
				ruizhi.ShowModalWin(url,width,heigth,paramObj,submitFn,modalClass);
			}else{
				ruizhi.Alert('请选择');
			}
			
		},
		
		//编辑窗口回调函数
		modifyComeBack : function(param){
			var params = {};
			param = ruizhi.ToJson(param);
			params.params = param;
			//alert("从模态窗口返回的参数："+ruizhi.ToJson(params));
			var url = WEB_ROOT + "/order/func/woDispRuleInst/createOrModify.do";
			ruizhi.InvokeMethodAsyn(url,params, ruizhi.order.woDispRuleInst.loadData);
			//ruizhi.order.woDispRuleInst.loadData();
		},
		
		//删除，可批量删除
		del : function(){
			var ids = grid1.getCheckedIds();//所选行的id数组
			if(!ruizhi.IsNull(ids) && ids.length > 0){
				ids = ids.join(",");
				var msg = "确定要删除吗？";
				bootbox.confirm(msg, function(result) {
					if(result || 'true' == result) {
						var params = {};
						params.ruleInstIds = ids;
						params.modifyUserId = _UserDetail.userId;//修改人
						//alert("从模态窗口返回的参数："+ruizhi.ToJson(params));
						var url = WEB_ROOT + "/order/func/woDispRuleInst/del.do";
						ruizhi.InvokeMethodAsyn(url,params, ruizhi.order.woDispRuleInst.loadData);
						//ruizhi.order.woDispRuleInst.loadData();
					} else {
						
					}
				});
			}else{
				ruizhi.Alert('请选择');
			}
		},
		
		
		//单选任务派发规则
		selWoDispRule : function(){
			var url = WEB_ROOT + '/order/funcPage/woDispRule/selWoDispRule.do';
			var width = null;
			var heigth = null;
			var paramObj = {};//传给模态窗口的参数
			paramObj.submitParam = null;//提交到后台的参数
			paramObj.title = "单选任务派发规则窗口";
			var submitFn = ruizhi.order.woDispRuleInst.selWoDispRuleComeBack;//模态窗口提交后回调方法
			var modalClass = 'modal-lg';
			ruizhi.ShowModalWin(url,width,heigth,paramObj,submitFn,modalClass);
		},
		
		selWoDispRuleComeBack :function(param){
			//alert(ruizhi.ToJson(param));
			if(!ruizhi.IsNull(param)){
				form1.setValue('ruleId', param.ruleId);
				form1.setValue('ruleType', param.ruleType);
			}
		},
		
		//字典表翻译字段值
		partyTypeTrans:function(cellvalue, options, rowObject){
			var talbeCode = "PF_WO_DISP_RULE_INST";
			var colCode = "PARTY_TYPE";
			var colValue = rowObject.partyType;
			var transName = ruizhi.TableDictTrans(talbeCode,colCode,colValue);
			return transName;
		},
		
	}

}();

//初始化
ruizhi.ExecWait(function() {
	ruizhi.order.woDispRuleInst.init();
});