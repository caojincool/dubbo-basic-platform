package org.lsmy.cloud.datasource;
 ...
 *       &#64;see org.lsmy.cloud.datasource.ReadWriteDataSourceDecision
 *       &#64;see org.lsmy.cloud.datasource.ReadWriteDataSource
 * 
 * </pre>
 * 
 *
...
public class ReadWriteDataSourceProcessor implements BeanPostProcessor {
	...
	private class ReadWriteDataSourceTransactionException extends NestedRuntimeException {
		private static final long serialVersionUID = -8230390412591317793L;
	...
	public Object determineReadOrWriteDB(ProceedingJoinPoint pjp) throws Throwable {
		if (isChoiceReadDB(pjp.getSignature().getName())) {
			...
			logger.debug("方法：{}进入读库！", pjp.getSignature().getName());
		} else {
			ReadWriteDataSourceDecision.markWrite();
			logger.debug("方法：{}进入写库！",pjp.getSignature().getName() );
		}
		...
		try {
			...
			logger.debug("{}reset方法",pjp.getSignature().getName());
	...
	private boolean isChoiceReadDB(String methodName) {
		...
		if (isForceChoiceRead == Boolean.TRUE) {
			logger.debug("表示强制选择 读 库");
			return true;
		}

		// 如果之前选择了写库 现在还选择 写库
		if (ReadWriteDataSourceDecision.isChoiceWrite()) {
			logger.debug("如果之前选择了写库 现在还选择 写库");
			return false;
		}

		// 表示应该选择读库
		if (isForceChoiceRead != null) {
			logger.debug("表示应该选择读库");
			return true;
		}
		// 默认选择 写库
		return false;
