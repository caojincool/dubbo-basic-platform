	...
	return this.each( function() {
				...
				if(ts.p.pginput===true) {
					if (ts.p.viewrecords){
						if(ts.p.reccount === 0) {
							...
							$("#input"+tspg).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.notext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>"));
						} else {
							from = base+1;
							tot=ts.p.records;
							if($.fmatter) {
							...
							var counttext=$.jgrid.template( rt ,from,to,tot);
							$("#input"+tspg).html($.jgrid.template($.jgrid.getRegional(ts, "defaults.pgtext", ts.p.pgtext) || "","<input "+getstyle(stylemodule, 'pgInput', false, 'ui-pg-input') + " type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>"+counttext));
		...
		setPager = function (pgid, tp){
			...
			$('.ui-pg-selbox',"#"+pgcnt).bind('change',function() {
				...
				//start 修改成可以查看全部数据，每页几条那里的下拉框有全部的选择 zzj 2017-11-16 16:24:30
...
//				return false;
				...
				//start 修改成可以查看全部数据，每页几条那里的下拉框有全部的选择 zzj 2017-11-16 16:27:25
			});
			if(ts.p.pgbuttons===true) {
		...
		$("th",thr).each(function ( j ) {
			...
			//start 修改不加载数据的时候也存在滚动条可以看table的标题 zzj 2017年5月3日14:11:37
//			firstr += "<td role='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'></td>";
			firstr += "<td role='gridcell' style='height:1px;width:"+w+"px;"+hdcol+"'></td>";
			//end 修改不加载数据的时候也存在滚动条可以看table的标题 zzj 2017年5月3日14:11:37
					...
					if(o.keys===true) {
						$(ind).bind( o.keyevent ,function(e) {
							...
							if (e.keyCode === 13) {
								...
								//修改回车换行 zzj 2016年8月23日14:48:36 start
								//$($t)就是table本身的对象
								if($($t) != null){
									var enterWrap = $($t).attr("enterWrap") == "true" ? true : false;//是否需要回车换行
									if(enterWrap){//回车换行
								...
								if(rowData != null){
									if(rowData.pageDateType != null && rowData.pageDateType == '' && rowData.pageDateType != undefined && rowData.pageDateType != 'undefined'){
										...
										$($t).jqGrid("setRowData",rowid,rowData);//保存的时候修改成UPDATE
									}
								}
								
								//var self = $("#"+rowid);//如果id里面有小数点的话，$符号不能拿到对象，需要对小数点进行转义
								var self = $(document.getElementById(rowid));
								var next_self = self.next("tr");
							    var next_rowid = next_self.prop('id');
							    //alert('下一行tr的rowid：'+next_rowid);
							    
							    var savesuccessfunc = self.parent().parent().attr("savesuccessfunc");//保存成功调用方法
							    
							    if(next_rowid != null && next_rowid != '' && next_rowid != undefined && next_rowid != 'undefined'){
							    	$($t).jqGrid('editRow', next_rowid, true);
									...
									next_rowid = randomId;
									//rowData['id'] = next_rowid;
							    	$($t).jqGrid('addRowData', next_rowid, rowData); // 增加下一行
							    	$($t).jqGrid('editRow', next_rowid, true);
							    	$($t).jqGrid("saveRow", next_rowid, null);
							    	if (savesuccessfunc != null && savesuccessfunc != 'undefined'){
							    		eval(savesuccessfunc + "()");
							    	}
							    	//$($t).jqGrid('editRow', next_rowid, true);
							    }
							    $($t).jqGrid('setSelection', next_rowid);
							    //修改回车换行 zzj 2016年8月23日14:48:42 end
